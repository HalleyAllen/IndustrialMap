...
- **国标行业分类库**：内置 GB/T 4754-2017 制造业完整三级分类（大类 31 + 中类 179 + 小类 609 = 819 项），一键写入数据库，企业可多选挂载
...
│   │   ├── data/                # 静态数据（随代码入库）
│   │   │   └── industry_categories.json  # GB/T 4754-2017 制造业分类（819 项）
│   │   └── routers/             # REST API 路由
│   │       ├── settings.py      # 数据库配置管理
...
| Method | Path | 说明 |
|---|---|---|
| POST | `/api/import/companies/preview` | 上传文件做自检（**不写库**），返回重复 / 无效行报告 |
| POST | `/api/import/companies/commit` | 确认后执行导入 |

两个端点均为 `multipart/form-data`，参数：

| 参数 | 必填 | 说明 |
|---|---|---|
| `file` | ✅ | CSV / TXT，UTF-8(BOM) / GB18030 自动识别，分隔符自动嗅探 |
| `default_theme_slugs` | | 默认主题，JSON 数组或逗号分隔，与行内主题取并集 |
| `on_duplicate` | | `skip`（默认）/ `update`（合并主题）/ `overwrite`（替换主题） |
| `read_theme_column` | | 是否读取第 2 列作为该行企业的主题，默认 `true` |

自检覆盖三类问题：**库内重复**（与已有企业同名）、**文件内重复**（仅首次出现计入）、**无效行**（名称为空或超过 200 字符）。
判定用「规范化名称」比对：去首尾空白 → 全角空格转半角 → 连续空白压缩 → 忽略大小写；原始名称始终原样入库。
